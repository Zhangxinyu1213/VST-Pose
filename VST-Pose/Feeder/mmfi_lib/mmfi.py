...
class MMFi_Dataset(Dataset):
    def __init__(self, data_base, data_unit, modality, split, data_form,window_size,step_size):
            ...
            for action in actions:
                if self.data_unit == 'sequence':
                    ...
                    for idx in range(frame_num):
                        ...
                        if data_valid:
                            data_info.append(data_dict)
                elif self.data_unit == 'window':
                    for start_idx in range(1, frame_num - window_size + 2, step_size):
                        data_dict = {'modality': self.modality,
                                     'scene': self.get_scene(subject),
                                     'subject': subject,
                                     'action': action,
                                     'idx_range': (start_idx, start_idx + window_size - 1),
                                     'gt_path': os.path.join(self.data_base.data_root, self.get_scene(subject), subject,
                                                             action, 'ground_truth.npy')
                                     }
                        data_valid = True
                        for mod in self.modality:
                            ...
                            for idx in range(start_idx, start_idx + window_size):
                                frame_path = os.path.join(self.data_base.data_root, self.get_scene(subject), subject,
                                                          action, mod, f"frame{idx:03d}{self.get_data_type(mod)}")
                            ...
                            if not data_valid:
                                break
                            data_dict[mod + '_paths'] = frame_paths
                        if data_valid:
                            data_info.append(data_dict)
                else:
    ...
    def read_sequence(self, frame_paths):
        """
        读取滑动窗口范围内的帧数据并组合为序列
        """
        data_mod_sequence = []
        for frame_path in frame_paths:
            ...
            if os.path.isfile(frame_path):
                data_mod_sequence.append(self.read_frame(frame_path))
            else:
                raise ValueError(f'{frame_path} is not a valid file!')
        return np.stack(data_mod_sequence)  # 将帧数据堆叠成序列
    ...
    def __getitem__(self, idx):
        ...
        if self.data_unit == 'sequence':
            ...
            sample = {'modality': item['modality'],
                      ...
                      'subject': item['subject'],
                      'action': item['action'],
                      'idx_range': item['idx_range'],
                      'output': gt_torch[item['idx_range'][0] - 1:item['idx_range'][1]]
                      }

            for mod in item['modality']:
                frame_paths = item[mod + '_paths']
                sample['input_' + mod] = self.read_sequence(frame_paths)
        else:
            raise ValueError('Unsupport data unit!')
        return sample
...
def make_dataset(dataset_root, config):
    ...
    a=config['window_size']

    train_dataset = MMFi_Dataset(database, config['data_unit'], **config_dataset['train_dataset'] , window_size=config['window_size'],step_size=config['step_size'])
    val_dataset = MMFi_Dataset(database, config['data_unit'], **config_dataset['val_dataset'] , window_size=config['window_size'],step_size=config['step_size'])
